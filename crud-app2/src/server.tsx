...
let selectedId = '';    // Holds the id of the currently selected dog

function dogRow(dog: Dog, updating = false) {
  // If the dog is being updated, we want to perfornm an out-of-band swap
  // SO a new table row can be replaced the existing one.
  ...
  return (
    <tr class="on-hover" id={`row-${dog.id}`} {...attrs}>
      ...
      <td>{dog.breed}</td>
      <td class="buttons">
        {/* Clicking this button askes the user if they really want 
            to delete the dog and does so if confirmed. */}
        <button
          class="show-on-hover"
        ...
        {/* Clicking this button selects the dog which triggers a
            selection-change event. That causes the form to update
